...
class MetaCorgiSnacks
  ...
  def self.define_snack(name)
    name = name.to_s
    define_method(name) {
      method_name_info = "get_#{name}_info"
      method_name_tastiness = "get_#{name}_tastiness"
      info = @snack_box.send(method_name_info, @box_id)
      tastiness = @snack_box.send(method_name_tastiness, @box_id)
